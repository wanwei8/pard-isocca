(function ($) {
    $.fn.page = function (options) {
        ...
        function ValidatePageCount() {
            if (settings.pageNo < 0 || settings.pageSize < 0 || settings.count < 0) {
                console.error("请检查 pageNo,pageSize,count 这三个参数是否存在非法输入");
                return false;
            }
        ...
        function PageDraw(json) {
            if (parseInt(settings.count) > 0) {
                ...
                for (var i in json.algorithm) {
                    if (json.algorithm[i].status == "disabled" || json.algorithm[i].status == "active") {
                ...
                if (settings.skipPart == true) {
                    $(that).find(".pagination").append('<span class="text-muted" style="margin:5px;margin-left:10px;display:inline-block;font-size:' + settings.fontSize + 'px;">共有' + pageCount + '页/' + settings.count + '个</span><div style="display:inline-block" name="changePage"><span class="text-muted" style="margin:5px;margin-left:0px;display:inline-block;font-size:' + settings.fontSize + 'px">，到第</span><input type="number" min="1" max="' + pageCount + '" class="pagination_change_page" style="width:45px;border-color:#ddd"><span class="text-muted" style="margin:5px;margin-left:10px;display:inline-block;font-size:' + settings.fontSize + 'px">页</span> <button class="btn btn-sm pagination_search">' + settings.btnName + '</button></div>');
                }
            } else {
